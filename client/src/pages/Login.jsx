...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setCargando(true);
    try { await login(form.email, form.password); window.location.href = '/biblioteca'; }
  ...
  return (
    <div className="flex items-center justify-center px-4 py-12">
      <div className="p-10 rounded-lg w-full max-w-md border-2 border-black shadow-[8px_8px_0px_#000000]" style={{ backgroundColor: colores.surface }}>
        <h1 className="text-4xl font-bold mb-8 text-center" style={{ color: colores.accent4, textShadow: '3px 3px 0px #000000' }}>INICIAR SESION</h1>
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <InputAccesible label="CONTRASENA" tipo="password" name="password" value={form.password} onChange={handleChange} error={errores.password} autoComplete="current-password" />
          {errorServidor && <p className="text-sm font-bold text-center" style={{ color: colores.error }} role="alert">{errorServidor}</p>}
          <BotonAccesible variante="primario" tipo="submit" className="w-full">{cargando ? 'ENTRANDO...' : 'ENTRAR'}</BotonAccesible>
        </form>
        <p className="mt-6 text-center font-bold" style={{ color: colores.text }}>
          NO TIENES CUENTA? <Link to="/registro" className="hover:underline" style={{ color: colores.accent5, textShadow: '2px 2px 0px #000000' }}>REGISTRATE AQUI</Link>
        </p>
      </div>
    </div>
  );
